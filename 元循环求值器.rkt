(define rule
  (list 
   ...
   (list assignment?         (lambda (exp env) (eval-assignment exp env)))
   (list definition?         (lambda (exp env) (eval-definition exp env)))
   (list if?                 (lambda (exp env) (eval-if exp env)))
   (list lambda?             (lambda (exp env) (make-procedure (lambda-parameters exp) (lambda-body exp) env)))
   (list begin?              (lambda (exp env) (eval-sequence (begin-actions exp) env)))
   (list cond?               (lambda (exp env) (eval (cond->if exp) env)))
   (list application?        (lambda (exp env) (my_apply (eval (operator exp) env)
...
;; 取得运算符
(define (operator exp) (car exp)) ;; 之前这里之成 (cdr exp) 所以操作符号写错了。。
;; 取得 运算对象 表
